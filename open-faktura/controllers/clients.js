...
const createClient = async (req, res) => {
    const {
        name, lastName, city, street
    } = req.body

    if (!name || !lastName || !street || !city) throw new BadRequest("Prosím vyplnte všetky hodnoty")

    if (!req.user.userId) throw new BadRequest("Zlý uživateľ")

    await Clients.create({userId: req.user.userId, ...req.body})

    res.status(OK).json({msg: "Client created!"})
}
...
const updateClient = async (req, res) => {
    ...
    if (!active) throw new BadRequest(`No client with id: ${setActive}`)
